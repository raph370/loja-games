...
while ($r = mysqli_fetch_assoc($items)) {
    $rows[] = $r;
    $total += $r['preco_final'] * $r['quantidade'];
}
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .logo { color:#e94560; font-size:1.5em; font-weight:700; letter-spacing:2px; text-decoration:none; }
        .logo span { color:#fff; }
        ...
        .item { background:#111118; border-radius:10px; padding:15px; margin-bottom:15px; display:flex; align-items:center; gap:15px; border:1px solid #1e1e2e; transition:border 0.2s; }
        .item:hover { border-color:#e94560; }
        .item img { width:100px; height:65px; object-fit:cover; border-radius:8px; flex-shrink:0; }
        .item-sem-img { width:100px; height:65px; background:#1e1e2e; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:2em; flex-shrink:0; }
        .item-info { flex:1; }
        .item-info h3 { color:#fff; margin-bottom:4px; font-size:1em; }
        .item-info p { color:#aaa; font-size:0.82em; margin-bottom:6px; }
        .preco { color:#ff6b6b; font-weight:700; font-size:1em; }
        .btn-remover { background:#e94560; color:#fff; padding:5px 12px; border-radius:6px; text-decoration:none; font-size:0.8em; border:none; cursor:pointer; transition:background 0.2s; }
        .btn-remover:hover { background:#c73652; }
        .total-box { background:#111118; border-radius:12px; padding:25px; text-align:center; margin-top:20px; border:1px solid #e94560; }
        .total-box h2 { color:#ff6b6b; font-size:1.8em; margin-bottom:20px; border:none; padding:0; }
        .btn-finalizar { background:#e94560; color:#fff; padding:14px 40px; border-radius:10px; text-decoration:none; font-size:1.1em; font-weight:700; display:inline-block; border:none; cursor:pointer; transition:background 0.2s; }
        .btn-finalizar:hover { background:#c73652; }
        ...
        .vazio a { color:#e94560; text-decoration:none; }
    ...
    <?php else: ?>
        ...
        <div class="item">
            ...
            if ($r['imagem'] && strpos($r['imagem'], 'http') === 0) {
                echo '<img src="'.htmlspecialchars($r['imagem']).'" alt="'.htmlspecialchars($r['nome']).'">';
            } elseif ($r['imagem']) {
                echo '<img src="imagens/'.htmlspecialchars($r['imagem']).'" alt="'.htmlspecialchars($r['nome']).'">';
            } else {
            ...
            <div class="item-info">
                ...
                <a href="carrinho.php?remover=<?=$r['id']?>" class="btn-remover" onclick="return confirm('Remover item?')">🗑 Remover</a>
            </div>
        </div>
        <?php endforeach; ?>
